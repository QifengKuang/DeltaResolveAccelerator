...
源码已加入强制重启后的自动恢复：启动界面或重新连接时核验上次会话，识别重启导致的网卡重编号和自动 IPv6 地址变化；真实残留及未解释配置差异仍会拦截，原始记录保持完整。详见[重启恢复说明](docs/TROUBLESHOOTING.md#重启后提示仍在运行或需要恢复)。这项修复同时依赖本提交的 UI 和后端，不包含在上方已有 1.1.0 轻量发布包中。

恢复检查也读取 Windows 原生路由来源，识别未重启期间已证实的 IPv6 内网路由通告变化，避免将正常的下一跳更新误报为加速残留。`Protocol=NetMgmt` 本身不作为自动路由的证据，完整快照和真实异常检查继续保留。
