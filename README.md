...
Windows x64 客户端源码，当前版本 **1.0.5**。使用腾讯云聚通 MNA 香港线路处理指定入口解析请求，对局流量保持本机直连。
...
界面采用暖白背景、浅薄荷绿连接卡片和深绿操作按钮，使用自绘圆角窗口栏；顶部空白区域可拖动，右上角可最小化或关闭。关闭仍会等待现有安全停止流程完成。
...
结果及截图位于 `tests/output/design/`，不会读取真实设备配置或启动加速。此检查是布局模拟，不改变 Windows 显示设置，也不替代跨显示器实测。
...
当前公开的是 1.0.5 源码，尚未在此仓库发布重新打包的安装程序。旧安装器和私人现场修复脚本没有纳入仓库。
