...
- 程序包包含界面、启动器和 12 个配套后端脚本；运行时、SDK、独立设备密钥需另行准备。已有安装也必须同步更新后端，见 [完整构建与部署说明](docs/BUILD.md)。
...
1.1.1 将正常关闭、同次开机异常恢复统一为当前资源释放检查：仅以 SDK 退出及本应用虚拟网卡、路由、DNS、代理等残留决定是否完成。Tailscale、WSL、Wi-Fi、DHCP 和外部配置变化仅作诊断；不再增加逐类快照豁免。读取暂态失败最多重试三次，原始快照保留，下一连接采集新基线。此判定不证明游戏延迟或普通网络连通性。

使用 `./build/Build-ApplicationBundle.ps1` 可隔离编译完整自有程序包，包含界面、启动器、配套后端和版本/源码/文件 SHA-256 清单；不会复制本机密钥、设置、日志或第三方运行时。详细命令及旧轻量更新通道限制见[完整构建说明](docs/BUILD.md)。
...
- 本分支修正已断开物理网卡的 DHCP 地址退回自动地址时，关闭加速后的恢复误报；使用前后完整快照及精确关联的本地路由判定，保留全部原始差异。见 [排查说明](docs/TROUBLESHOOTING.md)。
...
1.1.1 的恢复修复需要配套后端。提交并合并源码后，在干净工作树运行 `Build-ApplicationBundle.ps1` 和 `Test-ApplicationBundle.ps1`，核对清单中的源码提交与完整文件哈希，再发布整个程序包；详见 [构建说明](docs/BUILD.md)。
...
另一台电脑首次接入、重启后出现旧运行记录、移动安装目录后的恢复问题，见 [本机与远程排查流程](docs/TROUBLESHOOTING.md)。1.1.1 已整理本机验证过的动态资源检查和重启恢复修复；其他电脑仍需按其现场验证。更新前保留本机身份与设置，并逐项核对其他尚未提交的改动。
